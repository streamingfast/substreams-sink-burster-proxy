...
PROTO=${1:-"$ROOT/proto"}
PROTO_SUBSTREAMS=${2:-"$ROOT/../substreams/proto"}

function main() {
  checks

  set -e
  cd "$ROOT/proto/gen" &> /dev/null

  generate "stream.proto"
...
#  echo "streamingfast/substreams/proto revision: `GIT_DIR=$ROOT/.git git log -n 1 --pretty=format:%h -- proto`" >> ./last_generate.txt
}
...
function generate() {
    ...
    for file in "$@"; do
      protoc -I$PROTO -I$PROTO_SUBSTREAMS \
